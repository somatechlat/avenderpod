...
class ConsolidationConfig:
    ...
    replace_similarity_threshold: float = (
        0.75  # Threshold tuned for real cosine similarity scores
    )
...
class MemoryConsolidator:
    ...
    async def process_new_memory(
        ...
        try:
            ...
            result = await asyncio.wait_for(
                processing_task, timeout=self.config.processing_timeout_seconds
            )
            return result
            ...
            try:
                ...
                if log_item:
                    log_item.update(
                        ...
                        or "LLM analysis suggested skipping",
                    )
                return {"success": True, "memory_ids": [memory_id]}
            except Exception as e:
                    ...
                    for field_name, field_value in memory_metadata.items():
                        if field_name not in consolidated_metadata:
                            ...
                            if (
                                ...
                                merged_tags = list(
                                    set(consolidated_metadata[field_name] + field_value)
                                )
    ...
    async def _find_similar_memories(
        ...
        unique_similar.sort(
            key=lambda d: d.metadata.get("_consolidation_similarity", 0), reverse=True
        )
    ...
    async def _extract_search_keywords(
        self, new_memory: str, log_item: Optional[LogItem] = None
    ) -> List[str]:
    ...
    async def _apply_consolidation_result(
        ...
        try:
            ...
            consolidated_metadata = await self._gather_consolidated_metadata(
                db, result, original_metadata
            )

            # Handle each action type specifically
            if result.action == ConsolidationAction.KEEP_SEPARATE:
                return await self._handle_keep_separate(
