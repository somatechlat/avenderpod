...
class SaveInteractionRecord(Tool):
    ...
    parameters = {
        "type": "object",
        "properties": {
            "customer_wa_id": {"type": "string", "description": "WhatsApp number of the customer."},
            "archetype": {"type": "string", "description": "Type of interaction: 'food_order', 'medical_booking', 'lead'."},
            "payload": {"type": "string", "description": "JSON string containing all the interaction details (items, total, date, etc)."}
        },
        "required": ["customer_wa_id", "archetype", "payload"]
    }

    async def execute(self, customer_wa_id: str, archetype: str, payload: str, **kwargs):
        ...
        conn.commit()
        conn.close()
        return Response(message="Interaction successfully saved to the database. You may now confirm with the customer.", break_loop=False)
