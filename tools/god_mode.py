...
class GodMode(Tool):
    ...
    async def execute(self, **kwargs: Any) -> Response:
        ...
        if not tenant_id or not sysadmin_url:
            return Response(
                message="Error: System environment not configured for God Mode. Missing TENANT_ID or SYSADMIN_API_URL.",
                break_loop=True,
            )
        ...
        if action == "init":
            ...
            if not password or not pin:
                return Response(
                    message="Error: Both Master Password and Session PIN are required for verification.",
                    break_loop=False,
                )
            return await self._verify_challenge(
                sysadmin_url, tenant_id, str(password), str(pin), sysadmin_api_key
            )
        else:
            return Response(
                message=f"Error: Unknown action '{action}'. Use 'init' or 'verify'.",
                break_loop=False,
            )
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    return Response(
                        message=f"Failed to initiate challenge: {response.text}",
                        break_loop=True,
                    )
        except Exception as e:
            return Response(
                message=f"Error connecting to SysAdmin: {str(e)}", break_loop=True
            )

    async def _verify_challenge(
        self, base_url: str, tenant_id: str, password: str, pin: str, api_key: str
    ):
        url = f"{base_url}/auth/verify-challenge"
        payload = {"tenant_id": tenant_id, "password": password, "pin": pin}
        try:
            async with httpx.AsyncClient() as client:
                response = await client.post(
                    url,
                ...
                if response.status_code == 200 and data.get("ok"):
                    ...
                    return Response(
                        message=f"ACCESS DENIED: {data.get('message', 'Unknown error')}",
                        break_loop=False,
                    )
        except Exception as e:
            return Response(
