...
def file_tree(
    ...
    *,
    ...
    while queue and not limit_reached:
        ...
        folder_entries = [
            make_entry(folder, parent_node, level, "folder") for folder in folders
        ]
        file_entries = [
            make_entry(file_entry, parent_node, level, "file") for file_entry in files
        ]
...
class _TreeEntry:
    ...
    def as_dict(self) -> dict[str, Any]:
        return {
            ...
            "text": self.text,
            "items": (
                [child.as_dict() for child in self.items]
