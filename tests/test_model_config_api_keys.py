...
from plugins._model_config.extensions.python.banners import (
    _20_missing_api_key as missing_key_banner,
)
import models
...
def test_model_config_api_keys_can_be_cleared_via_backend(monkeypatch, tmp_path):
    ...
    assert handler._set_keys({"keys": {"openrouter": "sk-test-openrouter"}}) == {
        "ok": True
    }
...
def test_model_config_frontend_tracks_inline_api_key_edits():
    ...
    api_keys_mixin_path = (
        PROJECT_ROOT / "plugins" / "_model_config" / "webui" / "api-keys-mixin.js"
    )
    composer_store_path = (
        PROJECT_ROOT
        / "webui"
    ...
    model_field_path = (
        PROJECT_ROOT / "plugins" / "_model_config" / "webui" / "model-field.html"
    )
    modal_path = PROJECT_ROOT / "plugins" / "_model_config" / "webui" / "api-keys.html"

    store_content = store_path.read_text(encoding="utf-8")
    api_keys_mixin_content = api_keys_mixin_path.read_text(encoding="utf-8")
    model_config_content = f"{store_content}\n{api_keys_mixin_content}"
    composer_store_content = composer_store_path.read_text(encoding="utf-8")
    config_content = config_path.read_text(encoding="utf-8")
    model_field_content = model_field_path.read_text(encoding="utf-8")
    modal_content = modal_path.read_text(encoding="utf-8")

    assert "apiKeyDirty" in model_config_content
    assert "resetApiKeyDrafts()" in model_config_content
    assert (
        "!provider || seen.has(provider) || !this.apiKeyDirty[provider]"
        in model_config_content
    )
    assert "normalized[provider] = value.trim() ? value : '';" in model_config_content
