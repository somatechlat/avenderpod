...
def reset_tenant_state():
    ...
    try:
        subprocess.run([
            "docker", "exec", "avender_agent_zero", "python3", "-c",
...
def test_onboarding_wizard_and_catalog_ingestion():
    ...
    resp = requests.post("http://localhost:45001/api/plugins/avender/onboarding_api", json=payload)
    ...
    assert flag_proc.stdout.strip() == "true", "onboarding_complete flag not set to true"
