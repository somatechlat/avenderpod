...
def _get_named_exports(source: str) -> set[str]:
    ...
    exports.update(
        re.findall(r"^export\s+function\s+([A-Za-z0-9_]+)\s*\(", source, flags=re.M)
    )
    exports.update(
        re.findall(r"^export\s+const\s+([A-Za-z0-9_]+)\s*=", source, flags=re.M)
    )
    exports.update(
        re.findall(r"^export\s+class\s+([A-Za-z0-9_]+)\s*[\{:]", source, flags=re.M)
    )
...
def test_websocket_js_exports_minimal_namespaced_api_surface() -> None:
    source = (PROJECT_ROOT / "webui" / "js" / "websocket.js").read_text(
        encoding="utf-8"
    )
