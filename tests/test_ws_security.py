...
def test_open_handler_passes_even_without_tokens():
    assert (
        _check_security(
            _OpenHandler,
            _ctx(csrf_token=None, client_csrf_token=None, csrf_cookie=None),
        )
        is None
    )
...
def test_csrf_passes_with_all_tokens_matching(_mock):
    result = _check_security(
        _CsrfHandler, _ctx(csrf_token="abc", client_csrf_token="abc", csrf_cookie="abc")
    )
    assert result is None
...
def test_csrf_rejects_missing_server_token(_mock):
    result = _check_security(
        _CsrfHandler, _ctx(csrf_token=None, client_csrf_token="abc", csrf_cookie="abc")
    )
    assert result is not None
...
def test_csrf_rejects_missing_client_token(_mock):
    result = _check_security(
        _CsrfHandler, _ctx(csrf_token="abc", client_csrf_token=None, csrf_cookie="abc")
    )
    assert result is not None
...
def test_csrf_rejects_mismatched_client_token(_mock):
    result = _check_security(
        _CsrfHandler, _ctx(csrf_token="abc", client_csrf_token="xyz", csrf_cookie="abc")
    )
    assert result is not None
    assert result["code"] == "CSRF_INVALID"
...
def test_csrf_rejects_mismatched_cookie(_mock):
    result = _check_security(
        _CsrfHandler,
        _ctx(csrf_token="abc", client_csrf_token="abc", csrf_cookie="wrong"),
    )
    assert result is not None
...
@patch(
    "helpers.settings.get_settings", return_value={"mcp_server_token": "secret-key-123"}
)
