...
def test_ensure_bridge_dependencies_reinstalls_invalid_dependency_tree(monkeypatch):
    ...
    monkeypatch.setattr(
        bridge_manager, "NODE_MODULES_DIR", "/tmp/whatsapp-node-modules"
    )
    monkeypatch.setattr(
        bridge_manager.os.path,
        "isdir",
        lambda path: path == "/tmp/whatsapp-node-modules",
    )
    ...
    monkeypatch.setattr(
        bridge_manager, "_build_dependency_state", fake_build_dependency_state
    )
    monkeypatch.setattr(
        bridge_manager,
        "_load_dependency_state",
        lambda: {"package_json_hash": "a", "package_lock_hash": "b"},
    )
    monkeypatch.setattr(
        bridge_manager,
        "_validate_bridge_dependencies",
        fake_validate_bridge_dependencies,
    )
    monkeypatch.setattr(
        bridge_manager,
        "_reinstall_bridge_dependencies",
        fake_reinstall_bridge_dependencies,
    )
    monkeypatch.setattr(
        bridge_manager,
        "_write_dependency_state",
        lambda state: state_writes.append(state),
    )
...
def test_ensure_bridge_dependencies_bootstraps_missing_state_without_reinstall(
    monkeypatch,
):
    state_writes: list[dict] = []
    reinstall_calls: list[bool] = []
...
def test_reinstall_bridge_dependencies_uses_local_npm_cache(monkeypatch):
    ...
    monkeypatch.setattr(
        bridge_manager.PrintStyle, "warning", staticmethod(lambda message: None)
    )
    monkeypatch.setattr(
        bridge_manager.PrintStyle, "info", staticmethod(lambda message: None)
    )
    monkeypatch.setattr(
        bridge_manager, "NODE_MODULES_DIR", "/tmp/whatsapp-node-modules"
    )
    ...
    monkeypatch.setattr(
        bridge_manager.shutil, "rmtree", lambda path, ignore_errors=True: None
    )
...
def test_start_bridge_reinstalls_and_retries_after_dependency_failure(monkeypatch):
    ...
    monkeypatch.setattr(
        bridge_manager, "_ensure_bridge_dependencies", fake_ensure_bridge_dependencies
    )
